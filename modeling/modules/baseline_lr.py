...
from .hg_arch import HG
import matplotlib.pyplot as plt
from data.solver import get_gt_landmark_pair
import os
...
class Baseline_LR(nn.Module):
    ...
    def forward(self, y,z):  #  y:LR    z:LR->HR
        heatmap = self.HG(z)
        heat = heatmap

        sr1 = self.DIC(heatmap, y)

        sr2 = self.fuse(sr1,heatmap)
        global_feat = self.base(sr2)
        ...
        if self.training:
            return global_feat,feat,sr1,heat # global feature for triplet loss
        else:
            if self.neck_feat == 'after':
                # print("Test with feature after BN")
                return  feat, sr2, heat
            else:
                # print("Test with feature before BN")
                return global_feat,sr1,heat
